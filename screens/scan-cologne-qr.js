...
export default function ScanQr() {
  ...
  const handleBarCodeScanned = ({ type, data }) => {
    setScanned(true);
    alert(
      `Código qr tipo ${type} ubicación: latitude:${location.coords.latitude} longitude:${location.coords.longitude} y los datos del qr son: ${data} `
    );
  };
    ...
    <>
      <View style={styles.container}>
        ...
        <View className="absolute flex flex-row justify-around w-full bottom-24 z-30">
          <TouchableOpacity onTouchEndCapture={() => toggleFlash()}>
            <Ionicons
